...
const MOCK_CASES: CaseData[] = Array.from({ length: 45 }, (_, i) => ({
  id: `c${i + 1}`,
  patient: `Patient ${i + 1}`,
  description: `Cas médical généré #${i + 1}`,
  date: `2025-11-${(i % 30) + 1}`.padStart(10, "0"),
  statut: ["confirmé", "en_attente", "annulé"][i % 3] as CaseData["statut"],
}));

export default function CasMedicauxPage() {
  ...
  const filteredCases = useMemo(() => {
    return MOCK_CASES.filter(
      (c) =>
        c.patient.toLowerCase().includes(filter.toLowerCase()) ||
  ...
  const getStatusColor = (statut: CaseData["statut"]) => {
    switch (statut) {
      case "confirmé":
  ...
  return (
    <div className="p-6 mt-20 max-w-6xl mx-auto space-y-6">
      ...
      <div className="flex flex-col md:flex-row md:items-center md:justify-between gap-4">
        <h1 className="text-2xl font-bold text-primary">Cas médicaux</h1>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
        {paginatedCases.length === 0 ? (
          <p className="text-gray-500 col-span-full">Aucun cas trouvé.</p>
        ) : (
